---
title: "About"
description: "Some things to say about me."
showDate: false
showReadingTime: false
showWordCount: false
showAuthor: false
---

{{< lead >}}
Software developer and documentation specialist with more than a decade of hands-on experience.
{{< /lead >}}

Hi, I'm **adlerzei**. I work at
[Advancis Software & Services](https://www.advancis.net/) near Frankfurt, where I am a technical lead
for device integrations in WinGuard and the function owner for technical documentation. I hold a
Master's degree in IT Security and a Master's degree in Computer Science (completed with distinction),
both from TU Darmstadt.

<!-- TODO: add a few personal sentences, e.g. what you care about, hobbies. -->

## What I do

- **Software development**: C# and .NET, with a focus on integrating hardware and systems from many vendors.
- **Technical leadership**: coordinating a development sub-team and its technical decisions.
- **Documentation**: setting strategy, standards and tooling, including a Hugo-based DocOps toolchain with CI/CD.
- **Open source**: in my spare time I maintain browser extensions used by people around the world.
  Have a look at my [projects](/projects/).

## Get in touch

The easiest way to reach me is via [email](mailto:mail@adlerzei.de). You can also find me on
[GitHub](https://github.com/adlerzei), [LinkedIn](https://www.linkedin.com/in/christian-zei-567867181) and [Bluesky](https://bsky.app/profile/adlerzei).

If you like my work, you can support me on [Ko-fi](https://ko-fi.com/adlerzei).
